...
public class ParticipantServiceTest extends Mockito {
    ...
    public void createParticipant() {
        ...
        StudyParticipant participant = withParticipant()
                .withEnrollment(Enrollment.create(STUDY_ID,  EXTERNAL_ID))
        ...
        verify(enrollmentService).enroll(eq(account), enrollmentCaptor.capture());
    ...
    public void createParticipantWithExternalIdAndStudyCallerValidates() { 
        ...
        StudyParticipant participant = new StudyParticipant.Builder()
                .withEnrollment(Enrollment.create("study1", EXTERNAL_ID)).build();
    ...
    public void createParticipantWithExternalIdAndStudyCallerThatDontMatch() throws Exception { 
        ...
        StudyParticipant participant = new StudyParticipant.Builder()
                .withEnrollment(Enrollment.create("study2", "external-id")).build();
    ...
    public void createParticipantExternalIdNoPasswordIsUnverified() {
        ...
        StudyParticipant idParticipant = withParticipant().withPhone(null).withEmail(null).withPassword(null)
                .withEnrollment(Enrollment.create(STUDY_ID, EXTERNAL_ID)).build();
    ...
    public void createParticipantExternalIdAndPasswordIsEnabled() {
        ...
        StudyParticipant participant = withParticipant().withEmail(null).withPhone(null)
                .withEnrollment(Enrollment.create(STUDY_ID, EXTERNAL_ID))
                .withPassword(PASSWORD).build();
        participantService.createParticipant(APP, participant, false);
    ...
    public void createParticipantMustIncludeCallerStudy() {
        ...
        StudyParticipant participant = new StudyParticipant.Builder().copyOf(PARTICIPANT)
                .withEnrollment(Enrollment.create("inaccessible-study-to-caller", "externalId"))
    ...
    // The exception here results from the fact that the caller can't see the existence of the 
    // participant, because the study IDs don't overlap
    @Test(expectedExceptions = EntityNotFoundException.class)
    public void updateParticipantMustIncludeCallerStudy() {
        RequestContext.set(new RequestContext.Builder()
                .withOrgSponsoredStudies(ImmutableSet.of("someOtherStudy")).build());

        mockHealthCodeAndAccountRetrieval();
        
        StudyParticipant participant = new StudyParticipant.Builder().copyOf(PARTICIPANT)
                .withEnrollment(Enrollment.create("yetAnotherStudy", EXTERNAL_ID)).build();
    ...
    public void adminCanAddExternalIdOnCreate() {
        ...
        verify(accountService).createAccount(eq(APP), accountCaptor.capture(), eq(null));
        verify(enrollmentService).enroll(accountCaptor.capture(), enrollmentCaptor.capture());
        
        assertEquals(accountCaptor.getAllValues().get(0), accountCaptor.getAllValues().get(1));
    ...
    @Test(expectedExceptions = InvalidEntityException.class, 
            expectedExceptionsMessageRegExp=".*enrollment.studyId is not a study of the caller.*")
    public void studyResearcherCannotAddExternalIdOfOtherStudyOnCreate() {
        ...
        StudyParticipant participant = new StudyParticipant.Builder().copyOf(PARTICIPANT)
                .withEnrollment(Enrollment.create("someOtherStudy", "otherExternalId")).build();
        participantService.createParticipant(APP, participant, false);
